...
public class SampleModule extends ProcessModule {
	...
	public void execute() throws Exception {
		...
		String inputFile = Configuration.PROCESS_ROOT_FOLDER + "/" + this.getPath() + "/in-data.csv";
		...
		if (Files.exists(Paths.get(inputParamFile))) {
			...
			try {
				...
				logger.error("Impossible d'interprêter le taux d'échantillonnage passé en paramètre : " + params.containsKey("rate"));
			}
		}

		try {
			Stream<String> stream = Files.lines(Paths.get(inputFile)).filter(line -> random.nextFloat() < rate);
